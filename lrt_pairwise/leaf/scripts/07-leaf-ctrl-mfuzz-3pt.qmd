---
title: "Mfuzz clustering of leaf data, minimum 3 significant genes from point by point comparisons, control."
format: html
date: "2026-05-11"
...
original_names = ls(pattern = "leaf_ctrl_sig$")
...
metadata=data.frame(labelDescription=c("pooled sample", "letter", "tissue", "line", "hour of pooling", "sample number", "day", "replication", "condition", "id", "reads [million]", "million of reads binned", "grouping variable for averaging", "chronological time point", "group", "sizeFactor"), row.names=colnames(colData(vsd.sigs)))
...
save(vsd.eset.ave.std, file="../rdata-saved/vsd.eset.ave.std.leaf_ctrl_pair_min3pt.RDa")
```
...
```{r check random}
# RANDOM
# m-value must be at least 1.1

##############################
# Adjust parameters manually #
##############################

# c_val <- 10 ; m_val <- 1.05 # Non uniform partitioning (too low m value)
c_val <- 10 ; m_val <- 1.1 # produces uniform partitioning
...
load("../rdata-saved/vsd.eset.ave.std.leaf_ctrl_pair_min3pt.RDa")

## Parameters chosen by PI

#c_vals = c(8 : 15) # Dmin
c_vals = 14
m_vals = 1.25 # seq(1.15, 1.3, 0.05) # Dmin


## Uncomment on new session
# load("../rdata-saved/vsd.eset.ave.std.leafALLsig.RDa")
source("../../../vs-day-one/sam/scripts/mfuzz.plot2_separate_lines_ctrl.R") # modified plotting function, tailored to separate lines
...
source("../../../vs-day-one/sam/scripts/mfuzz.plot2_separate_lines_ctrl.R") # modified plotting function, tailored to seprarate lines
...
cl14_m1.25 <- results[["cl14_m1.25"]]

# this file contains only genes with mem >=0.7
genes_in_clusters <- lapply(1:ncol(cl14_m1.25$membership), function(i) {
  geny <- rownames(cl14_m1.25$membership)[cl14_m1.25$membership[, i] >= 0.7]
  return(geny)
})

# Name clusters
names(genes_in_clusters) <- paste("Klaster", 1:ncol(cl14_m1.25$membership))

save(cl14_m1.25, genes_in_clusters, file="../rdata-saved/mfuzz_cl14_m1.25_leaf.RDa")
