---
title: "GO analysis for expression changes in SAM in cold - ALL significant genes, control"
format: html
date: "2026-05-11"
...
# Wymuś ścieżkę do binarnego pliku kondy, jeśli R go nie widzi automatycznie
options(reticulate.conda_binary = "/home/mj/miniconda3/condabin/conda")

# force appropraite Conda enviromrnti - for goatools
...
expr.go.cats=read.table("../../../input-data/go-uniprot/SAM-ctrl.pop", header = FALSE, stringsAsFactors = FALSE)
...
Load the data from the mfuzz analysis for given number of clusters and fuzziness (computed in [07-SAM-ctrl-mfuzz-3pt.qmd](07-SAM-ctrl-mfuzz-3pt.qmd))
...
```{r save-for-09}
save(result.goseq, result.goseq25, result_clean, expr.go.cats, file="../rdata-saved/mfuzz_res13_for09_sam.RDa")
```
...
for (nm in names(result_clean)) {
  ...
  fname <- paste0("../results/go/graphs/clusters/all_cats/sam_ctrl_", gsub(" ", "_", nm), "summary.txt")
...
for (nm in names(result_clean)) {
  ...
  # file name – eg. "Klaster_13.txt"
  fname <- paste0("../results/go/graphs/clusters/all_cats/sam_ctrl_", gsub(" ", "_", nm), ".txt")
...
do
grep -f /media/mj/GLOWNY/project1-stat/input-data/go-uniprot/GO_BP_descendants.txt ${sample} > ../bp/${sample}.bp
grep -f /media/mj/GLOWNY/project1-stat/input-data/go-uniprot/GO_CC_descendants.txt ${sample} > ../cc/${sample}.cc
grep -f /media/mj/GLOWNY/project1-stat/input-data/go-uniprot/GO_MF_descendants.txt ${sample} > ../mf/${sample}.mf
done
```
...
for sample in ${sample_names[@]}
do
go_plot.py --obo=/media/mj/GLOWNY/project1-stat/input-data/go-uniprot/source-files/go.obo --go_file=${sample} --outfile=../png/bp/${sample}.png
done
...
for sample in ${sample_names[@]}
do
go_plot.py --obo=/media/mj/GLOWNY/project1-stat/input-data/go-uniprot/source-files/go.obo --go_file=${sample} --outfile=../png/cc/${sample}.png
done
...
for sample in ${sample_names[@]}
do
go_plot.py --obo=/media/mj/GLOWNY/project1-stat/input-data/go-uniprot/source-files/go.obo --go_file=${sample} --outfile=../png/mf/${sample}.png
done
```
