---
title: "Mfuzz clustering of SAM data in control, ALL significant genes"
format: html
date: "2026-02-04"
...
knitr::opts_chunk$set(root.dir = "/home/mj/project1-ctrl-stat/vs-day-one/sam/scripts")
```
...
mfuzz.plot2(vsd.eset.ave.std, cl=cl1, mfrow=c(4,4), x11 = FALSE)
dev.off()
```
...
# m=1.4 and c=24 # surely too many clusters - test for real data confirmed that
# m=1.45 and c=14 # too many clusters and too high fuzziness

# both ms seems too high
```
...
# m=1.15 and c=9
# m=1.2 and c=8 # maybe ok
# m=1.25 and c=10 # seems too fuzzy
# m=1.3 and c=10 # seems too fuzzy

# with 2 cluster margin (+/- max, min number from D min)
c_vals = 8 #seq(8, 10, 1)
m_vals = seq(1.15, 1.25, 0.05)


## Uncomment on new session
load("../rdata-saved/vsd.eset.ave.std.samALLsig.RDa")
source("mfuzz.plot2_separate_lines_ctrl.R") # modified plotting function, tailored to separate lines
...
c8_m1.15 <- results[["c8_m1.15"]]

# this file contains only genes with mem >=0.7
genes_in_clusters <- lapply(1:ncol(c8_m1.15$membership), function(i) {
  geny <- rownames(c8_m1.15$membership)[c8_m1.15$membership[, i] >= 0.7]
  return(geny)
})

# Name clusters
names(genes_in_clusters) <- paste("Klaster", 1:ncol(c8_m1.15$membership))

save(c8_m1.15, genes_in_clusters, file="../rdata-saved/mfuzz_c8_m1.15_samALLsig.RDa")
